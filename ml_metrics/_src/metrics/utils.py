...
def verify_input(y_true, y_pred, average, input_type, vocab, pos_label):
    ...
    Example:
      ...
      ... )
    """
    if average == types.AverageType.BINARY and input_type == types.InputType.BINARY:
        _validate_pos_label(y_true, y_pred, pos_label, vocab)


def _validate_pos_label(y_true, y_pred, pos_label: Any, vocab: dict[str, int] | None):
    vocab = vocab or classification.get_vocab(itertools.chain(y_true, y_pred), False)
