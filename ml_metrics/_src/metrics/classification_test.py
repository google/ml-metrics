...
class CalibrationHistogramTest(absltest.TestCase):
    ...
    def test_calibration_histogram(self):
        ...
        np.testing.assert_allclose(result.num_examples_hist, expected_num_examples_hist)
        np.testing.assert_allclose(result.labels_hist, expected_labels_hist)
        np.testing.assert_allclose(result.predictions_hist, expected_predictions_hist)
    ...
    @parameterized.named_parameters(
        [
            ...
            dict(
                ...
                expected_no_k_list=(math_utils.pos_sqrt(30) - 3)
                / 7,  # pyrefly: ignore[unsupported-operation]
                expected_with_k_list=[
                    3 / 8,
                    (math_utils.pos_sqrt(30) - 3) / 7,
