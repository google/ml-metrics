...
def avg_word_count(texts: Sequence[str]) -> stats.StatsState:
  """Average word count metric.

  Computes the average number of words within the texts. Characters that are not
  letters or spaces are taken out of the text, leaving only spaces between
  words.Note that each contraction is counted as only one word. For example,
  "I'm" is treated as one word, "Im".

  Args:
    texts:
      A non-vacant series of texts.
  ...
  batch_scorer_fn = pipeline.iterate_fn(text_scores.word_count)
  return aggregates.MergeableMetricAggFn(
      metric=stats.StatsState(batch_score_fn=batch_scorer_fn)
  )(texts)
...
def avg_token_count(
    texts: Sequence[str], tokenizer: Callable[[str], Sequence[Any]]
) -> float:
  """Average token count metric."""

  if not list(texts):
    raise ValueError('`texts` must not be empty.')

  batch_scorer_fn = pipeline.iterate_fn(
      lambda x: text_scores.token_count(x, tokenizer)
  )
  return aggregates.MergeableMetricAggFn(
      metric=stats.StatsState(batch_score_fn=batch_scorer_fn)
  )(texts)
