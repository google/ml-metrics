...
class UnboundesSamplerTest(absltest.TestCase):
    ...
    def test_with_transform_without_input_keys(self):
        ...
        self.assertEqual(
            expected_state, it.agg_state[metric_key]
        )  # pyrefly: ignore[unsupported-operation]
        self.assertEqual(
            ["a", "b", "c"], it.agg_result["samples"]
        )  # pyrefly: ignore[unsupported-operation]
    ...
    def test_with_transform_with_input_keys(self):
        ...
        _ = mit.last(it)
        self.assertEqual(
            [0, 1, 2, 3], it.agg_result["a"]
        )  # pyrefly: ignore[unsupported-operation]
        self.assertEqual(
            [10, 20], it.agg_result["b"]
        )  # pyrefly: ignore[unsupported-operation]
...
class FixedSizeSampleTest(parameterized.TestCase):
    ...
    def _assert_fixed_size_samples_equal(self, actual, expected):
        ...
        self.assertEqual(actual._num_samples_reviewed, expected._num_samples_reviewed)
    ...
    def test_fixed_size_sample_many_batches(self):
        size = (1000, 1000)
        datastreams = np.random.default_rng(0).uniform(low=-1e6, high=1e6, size=size)
        ...
        self.assertSequenceAlmostEqual(metric.result(), expected_reservoir, places=9)
    ...
    def test_sampling_add_uniformness(self):
        ...
        for i in range(num_runs):
            ...
            for v in sampler.result():  # pyrefly: ignore[not-iterable]
                actual_counter[
                    v
                ] += 1  # pyrefly: ignore[bad-index, unsupported-operation]
...
class CountTest(parameterized.TestCase):
    ...
    def test_count_fn(self):
        p = chainable.Pipeline().agg(stats.Count(batched_inputs=False, count_fn=len))
...
class MeanAndVarianceTest(parameterized.TestCase):
    ...
    def assertDataclassAlmostEqual(
        ...
        for key, value in expected.items():  # pyrefly: ignore[missing-attribute]
            try:
                ...
                self.fail(
                    f"Failed to assert {key}: {value} == {got[key]}"
                )  # pyrefly: ignore[bad-index]
    ...
    def test_mean_and_variance(self, num_batch, num_elements_per_batch, num_dimension):
        ...
        self.assertDataclassAlmostEqual(expected_last_batch_result, last_batch_result)
    ...
    def test_mean_and_variance_agg_fn(self):
        ...
        self.assertDataclassAlmostEqual(get_expected_mean_and_variance(batches), actual)
    ...
    def test_nested_agg_fn_nested(self, inputs, expected, batch_score_fn=None):
        ...
        test_utils.assert_nested_container_equal(self, expected, actual, strict=True)
...
class ValueAccumulatorTest(parameterized.TestCase):
    ...
    def test_value_accumulator_metric_fns(self):
        ...
        accumulator = stats.ValueAccumulator(concat_fn, metric_fns, batched_inputs=True)
        inputs = [[0, 1, 2], [3, 4, 5], [6, 7, 8]]
        for batch in inputs:
            accumulator.add(batch)
        actual = accumulator.result()
...
class R2TjurTest(parameterized.TestCase):
    ...
    def test_r2_tjur_many_batches_little_correlation(self, r2_metric, expected_result):
        np.random.seed(seed=0)
    ...
    def test_r2_tjur_many_batches_much_correlation(self, r2_metric, expected_result):
        np.random.seed(seed=0)
    ...
    def test_r2_tjur_absolute_many_batches_direct_correlation(self):
        ...
        expected_result = 1
        self.assertAlmostEqual(
            state.result(), expected_result, places=9
    ...
    def test_r2_tjur_many_batches_inverse_correlation(self, r2_metric, expected_result):
        y = np.round(np.random.uniform(size=(1000, 1000)))
        state = r2_metric()
        for y_i in y:
            state.add(y_i, 1 - y_i)
    ...
    def test_r2_tjur_relative_returns_nan(self, y_true, y_pred):
        self.assertTrue(math.isnan(stats.R2TjurRelative().add(y_true, y_pred).result()))
...
class RRegressionTest(parameterized.TestCase):
    ...
    def test_r_regression_one_large_batch(self):
        ...
        self.assertAlmostEqual(
            actual_result, expected_result, places=13
        )  # pyrefly: ignore[no-matching-overload]
    ...
    def test_r_regression_many_batches_little_correlation(self):
        ...
        self.assertAlmostEqual(
            state.result(), expected_result, places=14
        )  # pyrefly: ignore[no-matching-overload]
    ...
    def test_r_regression_many_batches_much_correlation(self):
        ...
        self.assertAlmostEqual(
            state.result(), expected_result, places=10
        )  # pyrefly: ignore[no-matching-overload]
