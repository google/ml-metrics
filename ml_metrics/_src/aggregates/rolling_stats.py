...
class FixedSizeSample(base.MergeableMetric):
  ...
  def _merge_reservoirs(self, other: Self) -> list[Any]:
  ...
  def add(self, inputs: types.NumbersT) -> Self:
    self._add_samples_to_reservoir(inputs, n=len(inputs))
    return self

  def merge(self, other: FixedSizeSample):
    # Directly extend the reservoir if combined reservoir is within the limit.
    if len(self._reservoir) + len(other.reservoir) <= self.max_size:
      self._reservoir.extend(other.reservoir)
    else:
