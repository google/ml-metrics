...
class Mean(base.MergeableMetric, base.CallableMetric):
  ...
  def _preprocess(self, batch: types.NumbersT) -> types.NumbersT:
    batch = self.batch_score_fn(batch) if self.batch_score_fn else batch
    batch = np.asarray(batch)
    if batch.size:
      self._input_shape = self._input_shape or batch.shape
      if not np.all(self._input_shape[1:] == batch.shape[1:]):
        raise ValueError(
            f'Incompatible input shape recieved {batch.shape=} while the'
            f' previous batches have shape {self._input_shape}.'
        )
    return batch
