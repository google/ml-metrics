...
class TopKWordNGramsTest(parameterized.TestCase):
    ...
    def test_topkwordngrams_invalid_kn(self):
        with self.assertRaisesRegex(ValueError, "k and n must be positive integers."):
...
class PatternFrequencyTest(parameterized.TestCase):
    ...
    def test_compute_pattern_frequency(self, batch, count_duplicate, expected_result):
    ...
    def test_compute_pattern_frequency_empty(self):
        metric = text.PatternFrequency(patterns=["ab", "xyx"], count_duplicate=False)
        batch_result = metric.add([])
        self.assertSequenceEqual([], batch_result)
    ...
    def test_pattern_frequency_add(self):
        ...
        self.assertSequenceAlmostEqual(expected_metric_result, sorted(metric.result()))
