...
class CoeffStateTest(parameterized.TestCase):
  ...
  def test_pearson_correlation_coefficient_squared_simple(self):
    x = (1, 2, 3, 4, 5, 6, 7)
    y = (10, 9, 2.5, 6, 4, 3, 2)

    actual_result = rolling_stats.PearsonCorrelationCoefficientSquaredAggFn()(
        x, y
    )
  ...
  def test_pearson_correlation_coefficient_squared_one_batch(self):
    np.random.seed(seed=0)
  ...
  def test_pearson_correlation_coefficient_squared_many_batches_little_correlation(
      self,
  ):
    np.random.seed(seed=0)
    x = np.array([
    ...
    state = (
        rolling_stats.PearsonCorrelationCoefficientSquaredAggFn().create_state()
    )
    for x_i, y_i in zip(x, y):
      rolling_stats.PearsonCorrelationCoefficientSquaredAggFn().update_state(
          state, x_i, y_i
      )

    actual_result = (
        rolling_stats.PearsonCorrelationCoefficientSquaredAggFn().get_result(
            state
        )
    )
  ...
  def test_pearson_correlation_coefficient_squared_many_batches_much_correlation(
      self,
  ):
    np.random.seed(seed=0)
    x = np.array([
  ...
  def test_pearson_correlation_coefficient_squared_many_batches_direct_correlation(
      self,
  ):
    x = np.array([
        np.random.uniform(low=-1e6, high=1e6, size=10000) for _ in range(10000)
  ...
  def test_pearson_correlation_coefficient_squared_many_batches_inverse_correlation(
      self,
  ):
    x = np.array([
        np.random.uniform(low=-1e6, high=1e6, size=10000) for _ in range(10000)
  ...
  def test_pearson_correlation_coefficient_squared_returns_nan(self, x, y):
    self.assertTrue(
        math.isnan(
            rolling_stats.PearsonCorrelationCoefficientSquaredAggFn()(x, y)
        )
    )
