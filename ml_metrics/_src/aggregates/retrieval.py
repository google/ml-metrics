...
def _precision(tp_at_topks, k_list, y_pred_count):
    return tp_at_topks[:, k_list - 1] / np.minimum(k_list, y_pred_count[:, np.newaxis])
...
class RetrievalMetricAtThreshold(str):
    ...
    This will parse a metric from a string to two parts of metric and threshold. The metric is one of the supported
    retrieval metrics. The threshold is a float number. If the threshold is not provided, it will be set to None.
    """
...
def retrieval_matcher(
    ...
    y_prob = y_prob or [None] * len(
        y_true
    )  # pyrefly: ignore[bad-argument-type, bad-assignment]
    for row_true, row_pred, row_prob in zip(
        y_true, y_pred, y_prob, strict=True
    ...
    def get_metric(self, metric: RetrievalMetricAtThreshold):
        if metric.threshold is None:
            ...
            return np.interp(  # pyrefly: ignore[no-matching-overload]
                metric.threshold,
                self.thresholds,
                getattr(self, metric.metric),  # pyrefly: ignore[bad-argument-type]
            )
...
class ThresholdedRetrieval(chainable.MergeableMetric):
    ...
    def __post_init__(self):
        thresholds = np.asarray(
            sorted(self.thresholds), dtype=np.float32
        )  # pyrefly: ignore[bad-argument-type]
    ...
    def add(
        ...
        y_probs = np.array(
            [y_prob > threshold for threshold in self.thresholds]
        )  # pyrefly: ignore[not-iterable]
...
class TopKRetrieval(chainable.MergeableMetric, chainable.HasAsAggFn):
    ...
    input_type: dataclasses.InitVar[InputType | str] = InputType.MULTICLASS_MULTIOUTPUT
