...
class FixedSizeSample(chainable.MergeableMetric, chainable.HasAsAggFn):
    ...
    def add(self, inputs: types.NumbersT):
        self._add_samples_to_reservoir(
            inputs, n=len(inputs)
...
class Histogram(chainable.CallableMetric, chainable.HasAsAggFn):
    ...
    _bin_edges: np.ndarray = dataclasses.field(default_factory=lambda: np.empty(0))
    ...
    def new(
        ...
        new_histogram, new_bin_edges = (
            np.histogram(  # pyrefly: ignore[no-matching-overload]
                inputs,
    ...
    def result(self) -> HistogramResult:
        return HistogramResult(hist=self._hist.copy(), bin_edges=self._bin_edges.copy())
...
class Count(chainable.CallableMetric):
    ...
    def new(self, inputs: types.NumbersT) -> types.NumbersT:
        ...
        if not self.batched_inputs:
            inputs = [inputs]  # pyrefly: ignore[bad-assignment]
        return dataclasses.replace(
            self, _count=sum(map(self.count_fn, inputs))
...
class Mean(chainable.CallableMetric):
    ...
    def new(self, batch: types.NumbersT) -> types.NumbersT:
        """Returns a new instance with the sufficient statistics reset and
        updated.
        ...
        Returns:
          Mean
        """
        batch = np.asarray(self.batch_score_fn(batch) if self.batch_score_fn else batch)
        return self.__class__(
    ...
    def total(self) -> types.NumbersT:
        return math_utils.where(
            self._count > 0, self._mean * self._count, 0
    ...
    def merge(self, other: Self) -> None:
        ...
        mean_diff = math_utils.nanadd(
            other.mean, -self._mean
        )  # pyrefly: ignore[unsupported-operation]
        update = mean_diff * math_utils.safe_divide(
...
class MinMaxAndCount(chainable.CallableMetric):
    ...
    def new(self, inputs: types.NumbersT) -> Self:
        ...
        return self.__class__(
            ...
            _min=(
                np.min(inputs, axis=self.axis) if inputs.size else np.inf
            ),  # pyrefly: ignore[bad-argument-type]
            _max=(
                np.max(inputs, axis=self.axis) if inputs.size else -np.inf
            ),  # pyrefly: ignore[bad-argument-type]
        )
...
class RRegression(chainable.CallableMetric):
    ...
    The Pearson correlation coefficient (PCC) is a correlation coefficient that measures linear correlation between two
    sets of data. It is the ratio between the covariance of two variables and the product of their standard deviations;
    thus, it is essentially a normalized measurement of the covariance, such that the result always has a value between
    -1 and 1. As with covariance itself, the measure can only reflect a linear correlation of variables, and ignores
    many other types of relationships or correlations. As a simple example, one would expect the age and height of a
    sample of teenagers from a high school to have a Pearson correlation coefficient significantly greater than 0, but
    less than 1 (as 1 would represent an unrealistically perfect correlation).
    ...
    def result(self) -> types.NumbersT:
        ...
        if self.center:  # Pearson's Correlation
            ...
            denominator_x = np.sqrt(
                self.sum_xx - self.sum_x**2 / self.num_samples
            )  # pyrefly: ignore[unsupported-operation]
