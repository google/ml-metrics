...
class ExampleCountAggFn(base.AggregateFn):
  ...
  def update_state(
    ...
    examples_len = sum(1 for _ in examples)
    weights_len = sum(1 for _ in example_weights)
    if examples_len != weights_len:
      raise ValueError(
          'examples and example_weights must have the same length, but'
          f' recieved {examples_len} examples and {weights_len} example'
          ' weights.'
      )
