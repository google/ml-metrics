...
def where(condition: agg_types.NumbersT, x: agg_types.NumbersT, y: agg_types.NumbersT):
    """Like np.where, but directly returns itself if condition is scalar."""
    if isinstance(condition, (list, tuple)) or (
        hasattr(condition, "__array__")
        and condition.ndim > 0  # pyrefly: ignore[missing-attribute]
    ):
