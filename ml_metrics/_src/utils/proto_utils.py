...
def tf_examples_to_dict(examples: _ExampleOrBytes):
    ...
    raise TypeError("Mutliple examples are not supported, got %s" % type(examples))
...
def dict_to_tf_example(data: dict[str, Any]) -> example_pb2.Example:
    ...
    for key, value in data.items():
        ...
        if isinstance(value[0], str):
            ...
            feature[key].bytes_list.value.extend(
                value
            )  # pyrefly: ignore[bad-argument-type]
        elif isinstance(value[0], (int, np.integer)):
            feature[key].int64_list.value.extend(
                value
            )  # pyrefly: ignore[bad-argument-type]
        elif isinstance(value[0], (float, np.floating)):
            feature[key].float_list.value.extend(
                value
            )  # pyrefly: ignore[bad-argument-type]
        else:
