...
The Complexity Gap Score (CG Score) quantifies the "influence of individual instances." CG scores identify observations
with large/small influence on downstream classification performance, potentially flagging label noise issues. The
current implementation works for binary labels only. More information about the stochastic method implemented here can
be found at
https://arxiv.org/pdf/2301.00930.pdf
(Section A.2).
"""
...
def complexity_gap_score(
    ...
    *,
    ...
    """Calculates the Complexity Gap (CG) score for identifying influential
    instances.
    ...
    data_by_label = _group_data_by_label(
        embeddings, labels
...
def _get_other_label(data_by_label: dict[int, object], current_label: int) -> int:
...
def _calculate_influence_scores(data: np.ndarray, data_size: int) -> types.NumbersT:
    ...
    hermitian = (hermitian_inner * (np.pi - np.arccos(hermitian_inner))) / (2 * np.pi)
